...
class PX4IMUBridge(Node):
    def __init__(self):
        super().__init__('px4_imu_bridge1')
        ...
        self.publisher = self.create_publisher(Imu, 'x500_drone_1/imu/data_converted', 10)

    def listener_callback(self, msg):
        imu_msg = Imu()
        imu_msg.header.stamp = self.get_clock().now().to_msg()
        imu_msg.header.frame_id = "x500_drone_1/imu_link" 
