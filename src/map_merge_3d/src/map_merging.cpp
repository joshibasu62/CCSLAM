...
#include <map_merge_3d/enum.h> // Ensure this is included for enums::from_string
#include "graph.h"
...
MapMergingParams MapMergingParams::fromROSNode(rclcpp::Node* node)
{
  MapMergingParams params;

  // Helper lambda for clean ROS 2 parameter loading
  ...
  get_param("resolution", params.resolution, 0.1);
  get_param("descriptor_radius", params.descriptor_radius, params.resolution * 8.0);
  get_param("outliers_min_neighbours", params.outliers_min_neighbours, 50);
  get_param("normal_radius", params.normal_radius, params.resolution * 6.0);

  // Keypoint Type (Using helper)
  std::string keypoint_type;
  get_param("keypoint_type", keypoint_type, std::string("SIFT"));
  if (!keypoint_type.empty()) {
    // This is safer than manual if/else because it uses the definition in enum.h
    params.keypoint_type = enums::from_string<Keypoint>(keypoint_type);
  }

  get_param("keypoint_threshold", params.keypoint_threshold, 5.0);

  // Descriptor Type (Using helper)
  std::string descriptor_type;
  get_param("descriptor_type", descriptor_type, std::string("PFH"));
  if (!descriptor_type.empty()) {
    params.descriptor_type = enums::from_string<Descriptor>(descriptor_type);
  }

  // Estimation Method (Using helper)
  std::string estimation_method;
  get_param("estimation_method", estimation_method, std::string("MATCHING"));
  if (!estimation_method.empty()) {
    params.estimation_method = enums::from_string<EstimationMethod>(estimation_method);
  }

  get_param("refine_transform", params.refine_transform, true);
  get_param("inlier_threshold", params.inlier_threshold, params.resolution * 5.0);
  get_param("max_correspondence_distance", params.max_correspondence_distance, params.inlier_threshold * 2.0);
  get_param("max_iterations", params.max_iterations, 500);

  int matching_k_temp = -1;
  get_param("matching_k", matching_k_temp, 5);
  if (matching_k_temp > 0) {
    params.matching_k = size_t(matching_k_temp);
  }

  get_param("transform_epsilon", params.transform_epsilon, 1e-2);
  get_param("confidence_threshold", params.confidence_threshold, 0.0);
  get_param("output_resolution", params.output_resolution, 0.05);
