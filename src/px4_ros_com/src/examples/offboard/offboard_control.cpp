...
//         current_goal_.x = 0;
//         current_goal_.y = 0;
//         current_goal_.z = -1.3;    //  Changed to 1.5m
...
//         // Rotation parameters
//         takeoff_altitude_ = -1.3;          // NED: -1.5 = 1.5m above ground
        ...
        auto timer_callback = [this]() -> void {
            ...
            if (offboard_setpoint_counter_ == 10) {
                ...
                if (!vehicle_status_.pre_flight_checks_pass ||
                    ...
                    offboard_setpoint_counter_ = 0;
                    RCLCPP_WARN(get_logger(),
                        "Offboard not set, waiting for preflight checks...");
                }
                else {
                    this->arm();
                }
...
private:
    ...
    void update_state()
    {
        // Periodic safety & joystick arm/disarm checks
        if (this->get_clock()->now() - last_request_ >
            rclcpp::Duration::from_seconds(1.0))
        {
            if (!vehicle_status_.pre_flight_checks_pass ||
                ...
                RCLCPP_ERROR(get_logger(),
                    "Pre-flight checks failing or offboard disabled!");
            }
            else if ((this->get_clock()->now() - twist_stamp_).seconds() < 1.0) {
                ...
                else if (vehicle_status_.arming_state ==
        ...
        // TAKING OFF → once altitude reached, go straight to HOVER
        if (control_State_ == kTakingOff) {
            double alt_error = std::abs(local_pose_.z - takeoff_altitude_);

            RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000,
                "Taking off... alt=%.2fm (target=%.2fm, error=%.2fm)",
                -local_pose_.z, -takeoff_altitude_, alt_error);

            if (alt_error < altitude_tolerance_) {
                RCLCPP_INFO(get_logger(),
                    "Reached %.2fm! Switching to position hold.",
                    -local_pose_.z);
                ...
                RCLCPP_INFO(get_logger(),
                    "Hovering at (%.2f, %.2f, %.2f) heading=%.1f°",
                    current_goal_.x, current_goal_.y, current_goal_.z,
                    current_goal_.heading * 180.0 / M_PI);
            }
        }
    ...
    void arm()
    {
        ...
        current_goal_ = local_pose_;
        current_goal_.z = takeoff_altitude_;
        current_goal_.heading = local_pose_.heading;  // Keep current heading

        RCLCPP_INFO(get_logger(), "Arming... takeoff to %.1fm at (%.2f, %.2f)",
            -takeoff_altitude_, current_goal_.x, current_goal_.y);

        control_State_ = kTakingOff;
        publish_vehicle_command(
            VehicleCommand::VEHICLE_CMD_COMPONENT_ARM_DISARM,
            VehicleCommand::ARMING_ACTION_ARM);
    }
    ...
    void publish_trajectory_setpoint()
    {
        ...
        switch (control_State_) {
            ...
            case kVelocityControl:
            {
                double time_since_last_twist =
                    (this->get_clock()->now() - twist_stamp_).seconds();
                ...
                msg.velocity[0] = twist_.linear.x * cosyaw
                                 - twist_.linear.y * sinyaw;
                msg.velocity[1] = twist_.linear.x * sinyaw
                                 + twist_.linear.y * cosyaw;
                msg.velocity[2] = velocity2d_ ? NAN : -twist_.linear.z;
    ...
    void twist_callback(const geometry_msgs::msg::Twist::SharedPtr msg)
    {
        ...
        if ((twist_stamp_ - arming_stamp_).seconds() > 5.0 &&
            control_State_ == kPositionControl)
        {
            current_goal_.z = local_pose_.z;
            if (std::abs(current_goal_.z) < 0.1) {
            ...
            RCLCPP_INFO(get_logger(),
                "Nav2 cmd_vel → velocity control (alt=%.2f)",
                current_goal_.z);
            control_State_ = kVelocityControl;
        }
    }
...
private:
    ...
    rclcpp::Subscription<VehicleLocalPosition>::SharedPtr
        local_pos_subscriber_;
    rclcpp::Subscription<VehicleStatus>::SharedPtr
        status_subscriber_;
    rclcpp::Subscription<VehicleCommandAck>::SharedPtr
        ack_subscriber_;
